...
class addResult{
    ...
    public function insertResult($resultData){
        ...
        // Check if email AND category combination already exists
        $checkSql = "SELECT id, wpm FROM " . $this->table_name . " WHERE email = ? AND category = ?";
        ...
        $checkStmt->bind_param("ss", $email, $category);
        $checkStmt->execute();
        $checkStmt->store_result();
        
        if ($checkStmt->num_rows > 0) {
            // Email and category combination exists, check if current WPM is higher
            ...
            // Email and category combination doesn't exist, insert new record
